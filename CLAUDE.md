...
Apple Watch health data — workouts and metrics, exposed as chart-ready series.

| | |
|---|---|
| Port | `9010` |
| Context path | `/saude-service` |
| Role required | `ROLE_SAUDE` |

Part of a personal microservices system; sibling repos live at `../sistema-*`. The API gateway fronts it at `https://sistema-backend.guilhermejr.net/saude-service`.
...
Weekly aggregations and 8-day moving averages are computed by **PostgreSQL views**, created in `V002` and `V003` — not in Java. Changing how a series is calculated usually means a new migration, not a service change.
